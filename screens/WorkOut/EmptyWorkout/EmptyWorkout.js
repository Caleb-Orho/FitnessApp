...
import { threedots, timer, trash, add2, dumbell } from "../../../assets/SVG";
...
export default function EmptyWorkout({ navigation }) {
    ...
    const finishWorkout = async () => {
        ...
        if (routineTitle === "") {
            ...
            setAlertText("Workout Title Cannot Be Empty")
            setAlertVisible(true);
            return
        }

        try {
            const directoryUri = FileSystem.documentDirectory + "/Workout/";
                ...
                <View className="flex flex-col items-center justify-center mt-5">
                    {routine.length == 0 ? (
                        <View className="flex flex-col items-center justify-center gap-2">
                            <Image source={dumbell} className='w-12 h-8' alt="add" />
